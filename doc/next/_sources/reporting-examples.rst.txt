 ...
 * Currently the API does not allow to filter on the current status of ads, which means the reporting numbers apply for **all** ads, including those that have been deleted in the meantime. Depending on API clients feedback, we might extend support for such filtering in the future.
