...
Any ads not present (referred to by **externalId**) in the XML file will be paused during
the import of this file.

The (mandatory) **externalId** field in the XML file is there to let us know, for consecutive imports, which
ads are the same. This results in allowing us to update an existing ad with the same **externalId** instead
of creating a new ad. **externalId** is mandatory and, as specified in the XSD, unique for each ad in the
XML file.

.. warning::
   The **externalId** field in feeds is the same field as **vendorId** in the sellside API
   and is, as in sellside API, unique per seller per ad. Our apologies for the wrong naming,
   we are aware this may cause confusion with using sellside API, where **externalId** has a
   different usage pattern than **vendorId**.
...
.. code-block:: xml
    ...
    <admarkt:ads xmlns:admarkt="http://admarkt.marktplaats.nl/schemas/1.0">
        <admarkt:ad>
            <admarkt:externalId>jh2fi82wqjckw</admarkt:externalId>
        ...
        <admarkt:ad>
            <admarkt:externalId>bvl202kfm2-123</admarkt:externalId>
