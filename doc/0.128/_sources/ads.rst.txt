...
:ref:`ad_postcode`                       string     --            max. 6 chars         no          yes
...
omitted. There is no unique constraint when it comes to externalId, so multiple
ads are allowed to have the same externalId.
...
This is not mandatory, and we will add the ad to the `default campaign` if not provided.
For the time being, we will also create the (default) campaign underwater if the seller has no campaign created yet, to ensure
...
A long value from the region tree. Can be the ID from any node.
