...
:ref:`ad_paypalEmail`               string     valid email        no          yes
=================================   ========   ================   =========   ========
...
per customer. This field is mandatory when creating an ad through the Sellside
API. However, it is not required when the ad is being created through one of
the other flows. When fetching an existing ad which was not created through
the Sellside API it is possible that the **externalId** field is empty or
omitted.
...
The `ISO 4217`_ currency code. Currently only *EUR* is supported. All Euro
amount values are stored in cents, i.e. EUR 10.50 are represented as 1050.
...
Suggested CPC value ine euro cents for this ad to be shown on page one.
...
A valid email address for the PayPal account of the seller.
